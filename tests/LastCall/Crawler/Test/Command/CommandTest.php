...
abstract class CommandTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function getMockHelperSet($configuration, $crawler, $profile = FALSE) {
        ...
        $crawlerHelper->getCrawler($configuration, $profile)->willReturn($crawler);

        $profilerHelper = $this->prophesize(ProfilerHelper::class);
        $profilerHelper->getName()->willReturn('profiler');
        if($profile) {
            $profilerHelper->renderProfile(Argument::type(OutputStyle::class))->shouldBeCalled();
        }
        else {
            $profilerHelper->renderProfile(Argument::type(OutputStyle::class))->shouldNotBeCalled();
        }
